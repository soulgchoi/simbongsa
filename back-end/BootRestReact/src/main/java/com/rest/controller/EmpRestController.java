...
import com.react.service.EmpService;
...
public class EmpRestController {
	...
	@ApiOperation("id 사원 검색") //
	public _Emp findEmpById(@PathVariable String id){
		try {
	...
	@ApiOperation("이름 사원 검색") //
	public List<_Emp> findLikeEmps(@PathVariable String name){
		try {
			return service.findLikeEmps(name);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	@ApiOperation("모든 부서 정보") //
	public List<_Dept> findAllDepts(){
		try {
			return service.findAllDepts();
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	@ApiOperation("모든 title 정보") //
	public List<_Emp> findAllTitles(){
		try {
	...
	@ApiOperation("새 사원 등록") //
	public Map addEmp(@RequestBody _Emp emp){
	...
	@ApiOperation("사원 정보 수정") //
	public Map updateEmp(@RequestBody _Emp emp){
		HashMap<String, String> map = new HashMap<>();
		try {
	...
	@RequestMapping(value="/emp/{id}", method=RequestMethod.DELETE) //메소드를 오퍼레이션으로 변경
	@ApiOperation("해당 번호의 고객 정보 삭제") //
	public Map deleteEmp(@PathVariable String id){ //url 경로상의 {num}을 가져오기 위한 어노테이션
		HashMap<String, String> map = new HashMap<>();
		try {
	...
	@ApiOperation("매니저의 사원 검색") //
	public List<_Emp> findEmpByMgrId(@PathVariable int managerId){
		try {
			return service.findEmpByMgrId(managerId);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	@ApiOperation("사원 부서 정보") //
	public List<_DeptEmp> findAllDeptEmps(){
		try {
			return service.findAllDeptEmps();
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	@ApiOperation("부서 이름 검색") //
	public List<_Emp> findDeptByname(@PathVariable String name){
		try {
			return service.findDeptByname(name);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	@ApiOperation("부서 번호 검색") //
	public List<_Emp> findDeptBydeptid(@PathVariable int dept_id){
		try {
			return service.findDeptBydeptid(dept_id);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
