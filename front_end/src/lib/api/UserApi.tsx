...
export const getUserFollower = async (userId: string) => {
  let response = await axios
    .get(restBaseApi + "follow/" + userId + "/followers", { headers: { "Authorization": token } });
...
export const getUserFollowing = async (userId: string) => {
  let response = await axios
    .get(restBaseApi + "follow/" + userId + "/followees", { headers: { "Authorization": token } });
  console.log("get follower", response);
  let list: string[] = [];
  const data = response.data.data;
  data.map((item: { m_userid: string }) => {
...
export const followUser = async (
  ...
  let response = await axios
    .post(restBaseApi + "insertfollow/", data, { headers: { "Authorization": token } });
...
export const unfollowUser = async (
  ...
  let response = await axios
    .post(restBaseApi + "deletefollow/", data, { headers: { "Authorization": token } });
  return response;
};
/// 팔로우 관련 API 끝
interface Iprefer {
...
export const localPreferRegister: ({
  ...
  userId
    ...
    try {
      console.log("체크 : ", data);
      return axios.patch(restBaseApi + `rest/Member/${userId}`, data, { headers: { "Authorization": token } });
    } catch (error) {
      return false;
    }
    // try {
...
export const localPreferInfo = (userId: string) => {
  try {
    ...
    return axios.get(restBaseApi + `rest/Member/${userId}/PreferDetail`, { headers: { "Authorization": token } });
  } catch (error) {
    console.log(error);
    return false;
  }
