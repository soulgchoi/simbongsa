...
class VolDetail extends React.Component<RouteComponentProps<any>, {}> {
  ...
  componentDidMount() {
    axios
      .get(this.state.url + this.state.v_id)
      .then(response => {
  ...
  render() {
    return (
      <div className="">
        ...
        <table className="">
          <tbody>
            <tr>
              <td>봉사기간</td>
              <td>
                {this.state.v_pBgnD.replace(/-/g, ".")} ~{" "}
                {this.state.v_pEndD.replace(/-/g, ".")}
              </td>
            </tr>
            <tr>
              <td>봉사시간</td>
              <td>
                {this.state.v_bgnTm.slice(0, 5)} ~{" "}
                {this.state.v_endTm.slice(0, 5)}
              </td>
            </tr>
            <tr>
              <td>모집기간</td>
              <td>
                {this.state.v_mBgnD.replace(/-/g, ".")} ~{" "}
                {this.state.v_mEndD.replace(/-/g, ".")}
              </td>
            </tr>
            <tr>
              <td>모집인원</td>
            ...
            <tr>
              <td>봉사자유형</td>
              <td>
                {this.state.v_adult === "Y" ? "성인" : null}{" "}
                {this.state.v_young === "Y" ? "청소년" : null}
              </td>
            </tr>
            <tr>
              <td>모집기관</td>
        ...
        <PostingButton v_id={this.state.v_id} />
      </div>
    );
  }
}
