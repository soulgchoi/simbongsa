...
class Map extends Component<IProps, IState> {
  ...
  updateDimensions = () => {
    this.setState({ width: window.innerWidth, height: (window.innerHeight - 435) });
  };
  // 화면 크기를 바꿀 때 높이 동적 반영에 필요한 코드
    ...
    return (
      <div className="map_wrap" id="map_wrap" style={{ height: height }}>
        ...
        <div className="custom_typecontrol radius_border">
          <span id="btnRoadmap" className="mylocation_btn" onClickCapture={() => { setMyLocation() }}>내위치</span>
        </div>
        <div className="custom_zoomcontrol radius_border">
          <span onClickCapture={() => { zoomIn(volMap) }}><img src="http://t1.daumcdn.net/localimg/localimages/07/mapapidoc/ico_plus.png" alt="확대" /></span>
          <span onClickCapture={() => { zoomOut(volMap) }}><img src="http://t1.daumcdn.net/localimg/localimages/07/mapapidoc/ico_minus.png" alt="축소" /></span>
        </div>
      </div>
    );
  }
