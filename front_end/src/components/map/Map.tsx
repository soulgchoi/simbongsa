...
class Map extends Component<Props, State> {
  ...
  render() {
    ...
    script.onload = () => {
      ...
      window.kakao.maps.load(function() {
        ...
        makeMarker(volunteers.toJS(), map); // (봉사정보 객체 리스트, 마커를 표시할 맵) 을 인풋으로 주면 맵에 마커가 표시됩니다.
  ...
  makeMarker = (volunteers: [], map: any) => {
    console.log("봉사지역들 : ", volunteers);
    const { kakao } = window;
    ...
    // positions[1].latlng = new kakao.maps.LatLng(37.4730725331551, 126.9);
    ...
    // 마커에 클릭이벤트를 등록합니다
    const { VolActions, clickedVolId, selectedVolunteer } = this.props;
    window.kakao.maps.event.addListener(marker, "click", function() {
      // 마커 위에 인포윈도우를 표시합니다
      VolActions.clickVol(1);
      console.log("선택된 아이디 : ", clickedVolId);
      VolActions.getVolById(clickedVolId);
      console.log("선택된 봉사활동 : ", selectedVolunteer.toJS());
...
export default connect(
  (state: any) => {
    return {
      ...
      selectedVolunteer: state.vol.get("selectedVolunteer")
    };
  },
  dispatch => ({
